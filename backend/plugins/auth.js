...
module.exports = fp(async function (fastify, opts) {
    ...
    // ── Super-Master & Privileged Super-Admins ──────────────────────────────
    fastify.decorate('requireSuperMaster', async function (request, reply) {
        try {
            ...
            const isSystemAdmin = request.user.role === 'SUPER_MASTER' || 
                                (request.user.role === 'SUPER_ADMIN' && privilegedIds.includes(request.user.studentId));

            if (!isSystemAdmin) {
                return reply.code(403).send({ error: 'Forbidden: Technical control access required' });
            }
            await checkForceLogout(request, reply);
        } catch (err) {
            if (!reply.sent) reply.code(401).send({ error: 'Unauthorized: Invalid or missing token' });
